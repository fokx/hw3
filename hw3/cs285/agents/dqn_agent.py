...
class DQNAgent(object):
  ...
  def step_env(self):
    ...
    perform_random_action = perform_random_action or self.t < self.learning_starts
    if perform_random_action:
      ...
      # I TODO: check whether `frame_history_len` is 1??
      # I think `obs_batch, act_batch, rew_batch, next_obs_batch, done_mask = buffer.sample(self.agent_params['frame_history_len'])` is the wrong one to use
